...
/* ======================================================================
   ...
   ----------------------------------------------------------------------
   tools/bake-textures.mjs writes a manifest whose entries are file NAMES
   relative to the manifest itself. This resolves them to URLs the engine can
   hand straight to an <img>, and degrades to `undefined` on any failure —
   a missing or broken manifest must never stop the notebook from mounting,
   it only means the device rasterises its own pages as before.
   ...
   BUCKETS (viewport sizes) and this loader:
     ...
     4. otherwise returns `undefined`, and the engine rasterises live, which is
        slower but always correct.
   That keeps one small manifest covering every device instead of one bake
   per CSS pixel size.
   ==================================================================== */
import type { BakedTextures } from '../types';
