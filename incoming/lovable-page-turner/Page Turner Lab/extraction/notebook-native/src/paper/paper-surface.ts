...
/* EXTRACTED from public/shared/paper-surface.js
   The drawing algorithms are untouched. What changed (baked-texture pass):
   the three canvas-generated layers (fibre, foxing, crease) can now resolve to
   a small library of STATIC PNGs shipped in /notebook-textures instead of
   being drawn per sheet and encoded as data URLs. Same artwork — the PNGs are
   produced by these very functions at build time (tools/bake-surfaces.mjs) —
   but the renderer decodes one shared file instead of ~60 unique data URLs. */
/* ======================================================================
   ...
   surface history, humidity as a neighbourhood episode.

   Every layer is a plain CSS background layer (gradients + image URLs), so
   the notebook's computed-style clone -> SVG foreignObject rasterisation
   path can carry it into the WebGL page-turn texture.
   ...
   sheet seen from the other side, so horizontal coordinates flip.
   ==================================================================== */
import { PaperStack } from './paper-stack';
const {rnd, clamp01, lerp, STOCK} = PaperStack;
/* the selected exposed cut-edge oxidation target (0.62) */
const EDGE_OX = PaperStack.EDGE_OXIDATION == null ? STOCK.oxidation : PaperStack.EDGE_OXIDATION;

/* ======================================================================
   BAKED TEXTURE LIBRARY
   ----------------------------------------------------------------------
   The per-sheet canvases differed only in a handful of continuous
   parameters, none of which the eye tracks precisely. Quantising them into
   a few steps turns ~60 unique rasterisations into a fixed, shareable set:

     fibre   4 strength steps                       -> 4 files
     foxing  4 density steps x 2 variants x mirror  -> 16 files
     crease  3 variants x mirror                    -> 6 files

   Both the baker and the runtime read the tables below, so a baked file and
   the live canvas it replaces are pixel-identical by construction.
   ==================================================================== */
export const FIBRE_STEPS = [0.35, 0.60, 0.80, 1.00];
export const FOX_STEPS   = [0.25, 0.50, 0.75, 1.00];
export const FOX_SEEDS   = [1301, 2617];          // two speck fields per density
export const CREASE_SEEDS = [101, 257, 613];      // three crease events
export const TEX_DIR = '/notebook-textures/';

const nearest = (steps, v) => {
  let bi = 0, bd = Infinity;
  for (let i = 0; i < steps.length; i++) { const d = Math.abs(steps[i] - v); if (d < bd) { bd = d; bi = i; } }
  return bi;
};
/* stable small hash so a given sheet always draws the same variant */
const hash = v => { let h = 2166136261 >>> 0; const s = String(v); for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 16777619) >>> 0; } return h; };

/* set by preloadPaperTextures(); null = draw live on the client */
let BAKED = null;
const bakedURL = name => `url("${BAKED.dir}${name}.${BAKED.ext}")`;

/**
 * Point the surface layers at the baked PNG library. Resolves true when the
 * manifest is present and the images decoded, false when it is not — in which
 * case every layer keeps being drawn live, exactly as before.
 */
export async function preloadPaperTextures(dir = TEX_DIR) {
  ...
  try {
    ...
    /* decode up front so the first page paint is a cache hit, not a network
       round trip inside the compositor's critical path */
    await Promise.all(manifest.files.map(name => new Promise(resolve => {
      const img = new Image();
      img.onload = img.onerror = () => resolve();
      img.src = dir + name + '.' + ext;
    })));
    BAKED = { dir, ext, ...manifest };
    SURFACE_CACHE.clear();
    return true;
  } catch { return false; }
}
export const usingBakedTextures = () => !!BAKED;
...
/* ---------- stock fibre ---------- */
export const FIBRE_SIZE = [200, 200];
export function drawFibre(g, w, h, seed, strength) {
  const r = rnd(seed);
...
function fibreTile(seed, strength) {
  if (BAKED) return bakedURL('fibre-' + nearest(FIBRE_STEPS, strength));
...
function drift(seed, amount) {
  ...
  for (let i = 0; i < 4; i++) {
    const a = 0.05 * amount + r() * 0.05 * amount;
    const l = blot(r() * 100, r() * 100, 45 + r() * 45, 40 + r() * 45, '176,150,102', a);
    if (a >= VISIBLE) L.push(l);
  }
  return L;
}
...
export const FOX_SIZE = [420, 560];
export function drawFoxing(g, w, h, seed, density, mir) {
  const r = rnd(seed);
  const X = x => (mir ? w - x : x);
...
function foxingTex(seed, density, mir) {
  if (BAKED) {
    const d = nearest(FOX_STEPS, clamp01(density));
    const v = hash(seed) % FOX_SEEDS.length;
    return bakedURL(`foxing-${d}-${v}${mir ? '-m' : ''}`);
  }
  return texURL('fox' + seed + '_' + density.toFixed(2) + (mir ? '_m' : ''), FOX_SIZE[0], FOX_SIZE[1],
    (g, w, h) => drawFoxing(g, w, h, seed, density, mir));
}

/* handling grime — where thumbs actually land: outer edge, mid height */
function grimeLayers(seed, amount) {
  if (amount * 0.07 < VISIBLE) return [];
  const r = rnd(seed + 3);
  return [
...
export const CREASE_SIZE = [320, 320];
export function drawCrease(g, w, h, seed, mir) {
  const r = rnd(seed);
...
function creaseTex(seed, mir) {
  if (BAKED) return bakedURL(`crease-${hash(seed) % CREASE_SEEDS.length}${mir ? '-m' : ''}`);
  return texURL('cr' + seed + (mir ? '_m' : ''), CREASE_SIZE[0], CREASE_SIZE[1],
    (g, w, h) => drawCrease(g, w, h, seed, mir));
}
...
export const PaperSurface = {
  ...
  preloadPaperTextures, usingBakedTextures,
};
