...
const PS = window.PaperStack;
if (!PS) { console.warn('[m2] /shared/paper-stack.js must load first'); return; }
...
const AGE = () => (window.PAPERV2 && window.PAPERV2.AGE != null) ? window.PAPERV2.AGE : 0.88;
...
(function injectCSS() {
  ...
  s.textContent =
    ...
    '.m2stack{position:relative;aspect-ratio:3/4;container-type:inline-size;--u:1cqw}' +
    '.m2stack>.m2sheet{position:absolute;top:7%;left:6%;height:86%;width:86%}';
...
function render(sp) {
  ...
  const surface = window.PaperSurface && window.PaperSurface.surface;
...
window.M2Stack = { CAND, COORD, spec, model, render, notebook, FLUSH_BOTTOM };
})();
